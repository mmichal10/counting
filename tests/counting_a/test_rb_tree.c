...
static inline uint32_t key(uint32_t raw)
{
	return raw << 6;
}
...
void test_root_remains_black_after_deleting_black_leaf(void)
{
	...
	size_t value_count = ARRAY_SIZE(values);
	struct rb_tree_node *root = NULL;
	...
	root = rb_tree_delete(root, values[0]);
	TEST_ASSERT_NOT_NULL(root);
	...
	for (size_t i = 0; i < ARRAY_SIZE(expected); i++)
		expected[i] = key(expected[i]);

	size_t expected_count = ARRAY_SIZE(expected);
...
void test_mixed_stress_insert_delete(void)
{
	...
	for (i = 0; i < 600; i++) {
		node = rb_tree_insert_and_fix_violations(root, key((uint32_t)i));
		TEST_ASSERT_NOT_NULL(node);
		root = rb_tree_get_root(node);
		present[i] = 1;
	...
	for (i = 600; i < 700; i++) {
		...
		root = rb_tree_delete(root, key(remove_key));
		if (root != NULL)
			root = rb_tree_get_root(root);
...
{
	uint32_t root_key = 10;
	root_key = key(root_key);
	struct rb_tree_node *root = rb_tree_insert_and_fix_violations(NULL, root_key);
	TEST_ASSERT_NOT_NULL(root);

	uint32_t left_key = 5;
	left_key = key(left_key);
	struct rb_tree_node *n5 = rb_tree_insert_and_fix_violations(root, left_key);
	...
	uint32_t right_key = 15;
	right_key = key(right_key);
	struct rb_tree_node *n15 = rb_tree_insert_and_fix_violations(root, right_key);
	...
	rb_tree_delete(root, right_key);
	root = rb_tree_get_root(root);

	TEST_ASSERT_NULL(rb_tree_find(root, right_key));

	struct rb_tree_node *n5_after = rb_tree_find(root, left_key);
...
void test_delete_black_node_with_single_red_child(void)
{
	...
	uint32_t child_key = 12;
	child_key = key(child_key);
	struct rb_tree_node *n12 = rb_tree_insert_and_fix_violations(root, child_key);
	TEST_ASSERT_NOT_NULL(n12);
	root = rb_tree_get_root(n12);

	struct rb_tree_node *target = rb_tree_find(root, right_key);
	...
	struct rb_tree_node *n12_after = rb_tree_find(root, child_key);
...
void test_delete_node_with_two_children(void)
{
	...
	uint32_t left_left_key = 3;
	left_left_key = key(left_left_key);
	struct rb_tree_node *n3 = rb_tree_insert_and_fix_violations(root, left_left_key);
	TEST_ASSERT_NOT_NULL(n3);
	root = rb_tree_get_root(n3);

	uint32_t left_right_key = 7;
	left_right_key = key(left_right_key);
	struct rb_tree_node *n7 = rb_tree_insert_and_fix_violations(root, left_right_key);
	TEST_ASSERT_NOT_NULL(n7);
	root = rb_tree_get_root(n7);

	struct rb_tree_node *target = rb_tree_find(root, left_key);
	TEST_ASSERT_NOT_NULL(target);
	TEST_ASSERT_NOT_NULL(target->left);
	TEST_ASSERT_NOT_NULL(target->right);
	TEST_ASSERT_EQUAL(target->color, RB_TREE_COLOR_BLACK);

	rb_tree_delete(root, left_key);
	root = rb_tree_get_root(root);

	TEST_ASSERT_NULL(rb_tree_find(root, left_key));

	struct rb_tree_node *n7_after = rb_tree_find(root, left_right_key);
	...
	struct rb_tree_node *n3_after = rb_tree_find(root, left_left_key);
	...
	struct rb_tree_node *n15_after = rb_tree_find(root, right_key);
...
{
	uint32_t root_key = 20;
	root_key = key(root_key);
	struct rb_tree_node *root = rb_tree_insert_and_fix_violations(NULL, root_key);
	TEST_ASSERT_NOT_NULL(root);
	root = rb_tree_get_root(root);

	uint32_t values[] = {10, 30, 5, 15, 25, 35};
	for (size_t idx = 0; idx < ARRAY_SIZE(values); idx++)
		values[idx] = key(values[idx]);
	...
	struct rb_tree_node *node20 = rb_tree_find(root, root_key);
	struct rb_tree_node *node10 = rb_tree_find(root, values[0]);
	struct rb_tree_node *node30 = rb_tree_find(root, values[1]);
	struct rb_tree_node *node5 = rb_tree_find(root, values[2]);
	struct rb_tree_node *node15 = rb_tree_find(root, values[3]);
	struct rb_tree_node *node25 = rb_tree_find(root, values[4]);
	struct rb_tree_node *node35 = rb_tree_find(root, values[5]);
	...
	root = rb_tree_delete(root, root_key);
	TEST_ASSERT_NOT_NULL(root);
	root = rb_tree_get_root(root);

	TEST_ASSERT_NULL(rb_tree_find(root, root_key));

	node25 = rb_tree_find(root, key(25));
	node10 = rb_tree_find(root, key(10));
	node30 = rb_tree_find(root, key(30));
	node5 = rb_tree_find(root, key(5));
	node15 = rb_tree_find(root, key(15));
	node35 = rb_tree_find(root, key(35));
...
void test_delete_black_node_with_red_successor(void)
{
	...
	uint32_t values[] = {10, 30, 25, 27, 35};
	for (size_t idx = 0; idx < ARRAY_SIZE(values); idx++)
		values[idx] = key(values[idx]);
	...
	struct rb_tree_node *node10 = rb_tree_find(root, values[0]);
	struct rb_tree_node *node30 = rb_tree_find(root, values[1]);
	struct rb_tree_node *node25 = rb_tree_find(root, values[2]);
	struct rb_tree_node *node27 = rb_tree_find(root, values[3]);
	struct rb_tree_node *node35 = rb_tree_find(root, values[4]);
	...
	root = rb_tree_delete(root, values[1]);
	TEST_ASSERT_NOT_NULL(root);
	root = rb_tree_get_root(root);

	TEST_ASSERT_NULL(rb_tree_find(root, values[1]));

	node20 = rb_tree_find(root, root_key);
	node10 = rb_tree_find(root, key(10));
	node25 = rb_tree_find(root, key(25));
	node27 = rb_tree_find(root, key(27));
	node35 = rb_tree_find(root, key(35));
...
{
	uint32_t root_key = 30;
	root_key = key(root_key);
	struct rb_tree_node *root = rb_tree_insert_and_fix_violations(NULL, root_key);
	TEST_ASSERT_NOT_NULL(root);
	root = rb_tree_get_root(root);

	uint32_t values[] = {15, 45, 10, 20, 40, 50, 60};
	for (size_t idx = 0; idx < ARRAY_SIZE(values); idx++)
		values[idx] = key(values[idx]);
	...
	struct rb_tree_node *node30 = rb_tree_find(root, root_key);
	struct rb_tree_node *node15 = rb_tree_find(root, key(15));
	struct rb_tree_node *node45 = rb_tree_find(root, key(45));
	struct rb_tree_node *node40 = rb_tree_find(root, key(40));
	struct rb_tree_node *node50 = rb_tree_find(root, key(50));
	struct rb_tree_node *node60 = rb_tree_find(root, key(60));
	struct rb_tree_node *node10 = rb_tree_find(root, key(10));
	struct rb_tree_node *node20 = rb_tree_find(root, key(20));
	...
	root = rb_tree_delete(root, key(40));
	TEST_ASSERT_NOT_NULL(root);
	root = rb_tree_get_root(root);

	TEST_ASSERT_NULL(rb_tree_find(root, key(40)));

	node30 = rb_tree_find(root, root_key);
	node15 = rb_tree_find(root, key(15));
	node45 = rb_tree_find(root, key(45));
	node50 = rb_tree_find(root, key(50));
	node60 = rb_tree_find(root, key(60));
	node10 = rb_tree_find(root, key(10));
	node20 = rb_tree_find(root, key(20));
...
void test_delete_root_with_deep_subtrees(void)
{
	...
	struct rb_tree_node *successor = rb_tree_find(root, key(45));
...
void test_delete_black_internal_node_with_red_sibling(void)
{
	...
	root = rb_tree_delete(root, key(10));
	TEST_ASSERT_NOT_NULL(root);
	root = rb_tree_get_root(root);
	...
	assert_tree_properties(root);
	assert_tree_matches(root, expected, expected_count);
	TEST_ASSERT_NULL(rb_tree_find(root, key(10)));

	struct rb_tree_node *parent = rb_tree_find(root, key(30));
	struct rb_tree_node *sibling = rb_tree_find(root, key(50));
...
void test_delete_black_node_with_cascading_double_black(void)
{
	...
	root = rb_tree_delete(root, key(25));
	TEST_ASSERT_NOT_NULL(root);
	root = rb_tree_get_root(root);
	...
	assert_tree_properties(root);
	assert_tree_matches(root, expected, expected_count);
	TEST_ASSERT_NULL(rb_tree_find(root, key(25)));

	struct rb_tree_node *replacement = rb_tree_find(root, key(27));
...
void test_delete_black_node_requiring_dummy_leaf(void)
{
	...
	struct rb_tree_node *node30 = rb_tree_find(root, key(30));
	TEST_ASSERT_NOT_NULL(node30);
	TEST_ASSERT_EQUAL(RB_TREE_COLOR_BLACK, node30->color);
...
void test_sequential_deletions_triggering_rotations(void)
{
	...
	size_t case_count = ARRAY_SIZE(cases);

	for (size_t c = 0; c < case_count; c++) {
		cases[c].key = key(cases[c].key);
		for (size_t j = 0; j < cases[c].expected_count; j++)
			cases[c].expected[j] = key(cases[c].expected[j]);
	}

	for (size_t idx = 0; idx < case_count; idx++) {
		root = rb_tree_delete(root, cases[idx].key);
		TEST_ASSERT_NOT_NULL(root);
		root = rb_tree_get_root(root);
		assert_tree_properties(root);
		assert_tree_matches(root, cases[idx].expected, cases[idx].expected_count);
		TEST_ASSERT_NULL(rb_tree_find(root, cases[idx].key));
	}
